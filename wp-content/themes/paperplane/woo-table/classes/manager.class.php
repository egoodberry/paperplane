     ...
     if ( is_page( $this->manage_page ) ) {
      ...
      if ( isset( $_email ) && is_email( $_email ) ) {
       ...
       if ( $date_format == '' ) { $date_format = 'jS F Y'; } // End IF Statement
        ...
        //paperplane
       $content .= '<div class="container"><div class="row"><div class="grid_12">';
         ...
         $content .= '<tbody>' . "\n";
          ...
          $content .= '<tr>' . "\n";
           ...
           $content .= '<td>' . "\n";
             ...
             // Cater for users navigating in a non-stored session.
             $email_and_key = '';
             if ( isset( $_GET['e-mail'] ) && isset( $_GET['key'] ) && md5( urldecode( $_GET['e-mail'] ) ) == $_GET['key'] ) {
              ...
              $email_and_key = '&e-mail=' . $_email . '&key=' . $_key;
             } else {
              if ( isset( $_POST['e-mail'] ) && isset( $_POST['key'] ) && md5( urldecode( $_POST['e-mail'] ) ) == $_POST['key'] ) {
               $_email = urlencode( $_POST['e-mail'] );
               $_key = urlencode( $_POST['key'] );
             ...
             if ( $r['reservation_status'] == 'unconfirmed' ) {
              $content .= '<a class="confirm" href="' . get_permalink( $this->manage_page ) . $_concatenator . 'action=confirm&id=' . $r['ID'] . $email_and_key . '"><span>' . __( 'Confirm', 'woothemes' ) . '</span></a>';
             } // End IF Statement

             if ( $r['reservation_status'] !== 'cancelled' ) {
              $content .= '<a class="update" href="' . get_permalink( $this->booking_page ) . $_concatenator . 'action=update&id=' . $r['ID'] . $email_and_key . '">' . __( 'Update', 'woothemes' ) . '</a>';
             } // End IF Statement

             if ( $r['reservation_status'] !== 'cancelled' ) {
              $content .= '<a class="cancel" href="' . get_permalink( $this->manage_page ) . $_concatenator . 'action=cancel&id=' . $r['ID'] . $email_and_key . '">' . __( 'Cancel', 'woothemes' ) . '</a>';
             } // End IF Statement
