	...
	class WooTable_PostType_Reservation {
		...
		?>
		    		...
		    		<select name='wootable_date_filter' id='wootable_date_filter' class='postform alignleft'>
	        			...
	        			<option value="" <?php if( isset($_GET['wootable_date_filter']) && $_GET['wootable_date_filter']=='' ) echo 'selected="selected"' ?>><?php _e('All Reservations','woothemes'); ?></option>
					...
					<?php /*<input type="submit" name="submit" value="<?php _e('Filter','woothemes'); ?>" class="button-secondary action alignleft" />*/ ?>
			<?php
		...
		public function posts_where ( $where ) {
		   ...
		   $where .= " AND meta.meta_key = 'reservation_date'";
		    ...
		    if( isset( $_GET['wootable_date_filter'] ) && $_GET['wootable_date_filter'] == 'upcoming' && is_admin() && $_GET['post_type'] == $this->token ) {
		...
		public static function add_custom_column_data ( $column_name, $id ) {
			...
			switch ($column_name) {
				...
				case 'reserveddatetime':
					...
					$date_formatted = date_i18n("jS F Y", mktime(0, 0, 0, $date_bits[1], $date_bits[2], $date_bits[0] ));
