...
class WTFactory {
  ...
  public static function get_business_hours_display ( $plugin_prefix, $is_twelvehour = false ) {
    ...
    foreach ( $_business_hours as $k => $v ) {
      ...
      */
        if ( isset( $_closed_hours[$k] ) && ( count( $_closed_hours[$k] ) > 0 ) ) {
          ...
          if ( $current_day == 'tue' ) { $current_day = 'tues'; }
          if ( $current_day == 'thu' ) { $current_day = 'thurs'; }
            $current_day_num = 0;
          $main_day_num = 0;
          foreach ( $day_num as $i => $day ) {
            if ( $day == $current_day ) { $current_day_num = $i; }
              if ( $k == $day ) { $main_day_num = $i; }
          }
          ...
          $date = date_i18n( 'Y-m-d', mktime( 0, 0, 0, date( 'm', $current_timestamp ), date( 'd', $current_timestamp ) + $result, date( 'Y', $current_timestamp ) ) );
       ...
       */
          if ( count( $times ) > 1 ) {
            for ( $i = 0; $i < count( $times ); $i++ ) {
              ...
              if ( $is_twelvehour ) {
                ...
                $_time = date_i18n( 'h:i A', mktime( $_hour, $_min, 0, 12, 32, 1997 ) );
                $times[$i] = $_time;
              }
            } // End FOR Loop
            for ( $i = 0; $i < count( $times ); $i++ ) {
        ...
        */
          ...
          if ( $is_twelvehour ) {
            ...
            $_closing_hour = substr( $v['closingtime'], 0, 2 );
            $_closing_min = substr( $v['closingtime'], 3, 2 );
            $_opening = date_i18n( 'h:i A', mktime( $_opening_hour, $_opening_min, 0, 12, 32, 1997 ) );
            $_closing = date_i18n( 'h:i A', mktime( $_closing_hour, $_closing_min, 0, 12, 32, 1997 ) );
          }
          $_value = $_opening . ' - ' . $_closing; // Used before we added the "closed hours" functionality.
        }
      }
    ...
    if ( $_times_display ) {
      ...
      foreach ( $_times_display as $_t ) {
        ...
        $class = '';
        if ( $_t['value'] == __( 'Closed', 'woothemes' ) ) { $class = 'closed'; } else { $class = 'open'; }
  ...
  public static function display_changed_times ( $plugin_prefix, $echo, $time, $page_id = 0, $date = '', $is_admin = false ) {
    ...
    if ( $date == '' ) { $date = $_POST['reservation_date']; }
    ...
    if ( $business_hours ) {
      ...
      if ( $index == 'thu' ) { $index = 'thurs'; }
      if ( $index == 'tue' ) { $index = 'tues'; }
  ...
  public static function get_times_between ( $start, $finish, $plugin_prefix, $date, $time_to_skip = '', $exclude_closed_hours = false ) {
    ...
    // Compensate for the colloquial convention of "thurs" instead of "thu", and "tues" instead of "tue".
    if ( $_today_day == 'thu' ) { $_today_day = 'thurs'; }
    if ( $_today_day == 'tue' ) { $_today_day = 'tues'; }
      if ( $_yesterday_day == 'thu' ) { $_yesterday_day = 'thurs'; }
        if ( $_yesterday_day == 'tue' ) { $_yesterday_day = 'tues'; }
    ...
    if ( $_yesterday_hours['closingtime'] <= $_beforenoon ) {
      ...
      for ( $i = 0; $i <= $_yesterday_hour; $i++ ) {
        ...
        if ( $full_hour < $_yesterday_hours['closingtime'] ) { $_aftermidnight[] = $full_hour; }

        // Don't add the extra $half_hour for the end time.
        if ( $_interval == '0.5' ) {
          if ( $half_hour == $_yesterday_hours['closingtime'] ) {} else { $_aftermidnight[] = $half_hour; }
        }
      }
    }
    ...
    for ( $i = $start; $i <= $finish; $i += $_interval) {
      $hour = $i;

      if ( $hour + $_interval > $finish ) {
        continue;
      }
